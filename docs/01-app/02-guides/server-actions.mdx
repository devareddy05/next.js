...
This pairs naturally with [`useActionState`](https://react.dev/reference/react/useActionState):
...
export async function stepAction(_prev, formData) {
...
export function LoginForm() {
  const [node, formAction] = useActionState(stepAction, <EmailStep />)
  return <form action={formAction}>{node}</form>
}
```
